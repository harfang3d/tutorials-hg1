...
shader_color = egl.LoadShader(os.path.join(os.getcwd(), "../_data/shader_2d_color.isl"))
shader_texture = egl.LoadShader(os.path.join(os.getcwd(), "../_data/shader_2d_single_texture.isl"))
...
while egl.GetDefaultOutputWindow():
	...
	egl.Clear(gs.Color.Green)  # green
	...
	egl.SetRenderTarget(None)

	window_size = egl.GetDefaultOutputWindow().GetSize()
	egl.SetViewport(gs.fRect(0, 0, window_size.x, window_size.y))  # fit viewport to window dimensions
	egl.Clear(gs.Color.Red)  # red
